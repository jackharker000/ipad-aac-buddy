...
- **6 June 2026 — iPad autologin via device keys.** The 5 June stay-logged-in pass closes the within-7-day-window case (IDB persistence + start_url=/app + refresh-on-resume). For the past-7-day case — where Apple's ITP has wiped script-writable storage entirely — we add an opt-in long-lived device key that the user bakes into a home-screen icon via a one-shot dynamic manifest swap. Settings → System → "iPad home-screen launch" mints a key (server-side, 32 bytes random → returned exactly once), client-side rewrites `<link rel="manifest">` to `/api/manifest?key=…`, the user taps Add-to-Home-Screen RIGHT NOW, iOS bakes `/app?device_key=…` into the icon. Future taps land on /app; `useSession` reads the param off the URL, posts to `/api/autologin`, which validates against Firestore (storing `SHA-256(key)` as the doc id so a DB breach reveals only mappings, not usable keys) and mints a Firebase custom token; `signInWithCustomToken` exchanges it for a real session, `onAuthStateChanged` fires, the cockpit renders. The URL is stripped via `history.replaceState` after consumption so refreshes don't re-fire. Trade-off acknowledged in the UI: the home-screen icon skips all sign-in prompts — anyone with the iPad gets in. Each key is labelled, lists last-used timestamp, and is revocable from Settings; revocation deletes the Firestore doc and the icon becomes inert immediately. Top-level `deviceKeys/{hashId}` collection is server-write-only (security rule in `docs/setup.md`); every client touch goes through `/api/device-keys/{create,list,revoke}` with an ID token, and `/api/autologin` uses the device key as the credential.

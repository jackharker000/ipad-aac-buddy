...
- **Setting — `cloudSyncEnabled`** (default `true`). Surfaced as a per-account toggle in Settings. When `false`, the sync engine pauses outbound writes for that account and the cockpit stays local-first. Flipping back to `true` doesn't backfill historical data — sync is new-only.
